...
function setup_vendor() {
    ...
    export PRODUCTMK="$CANDY_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$CANDY_ROOT"/"$OUTDIR"/Android.mk
    export ANDROIDBP="$CANDY_ROOT"/"$OUTDIR"/Android.bp
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            ...
            printf "$COMMENT_PREFIX Copyright (C) $YEAR CandyRoms\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "$COMMENT_PREFIX Copyright (C) 2017-$YEAR CandyRoms\n" >> $1
        else
            printf "$COMMENT_PREFIX Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR CandyRoms\n" >> $1
        fi
    else
        printf "$COMMENT_PREFIX Copyright (C) $YEAR CandyRoms\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$PATCHELF" ]; then
        export PATCHELF="$CANDY_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/patchelf
    fi
